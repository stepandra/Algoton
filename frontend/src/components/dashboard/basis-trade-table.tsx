    ...
    return (
        <Card className="border-1 border-white/5 bg-zinc-900/50 flex flex-col h-full overflow-hidden">
            <CardHeader className="py-2 px-4 border-b border-white/5 h-10 min-h-0 flex-shrink-0 flex flex-row items-center justify-between">
                <CardTitle className="text-sm font-medium text-zinc-400">{UI_TEXT.titles.basisTradeOpportunities}</CardTitle>
                ...
                <Table>
                    <TableHeader className="bg-zinc-900/90 sticky top-0 z-10">
                        <TableRow className="border-white/5 hover:bg-transparent">
                            <TableHead className="text-xs font-semibold text-zinc-500 uppercase w-[100px]">{UI_TEXT.labels.time}</TableHead>
                            <TableHead className="text-xs font-semibold text-zinc-500 uppercase">{UI_TEXT.labels.symbol}</TableHead>
                            <TableHead className="text-xs font-semibold text-zinc-500 uppercase text-right text-emerald-500">{UI_TEXT.labels.buyOnDeDust}</TableHead>
                            <TableHead className="text-xs font-semibold text-zinc-500 uppercase text-right text-red-500">{UI_TEXT.labels.shortOnPerps}</TableHead>
                            <TableHead className="text-xs font-semibold text-zinc-500 uppercase text-right">{UI_TEXT.labels.spreadPercent}</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {data.length === 0 ? (
