            ...
            <div className="p-2 border-b border-border flex items-center justify-between bg-zinc-900/50">
                <div className="flex items-center gap-4">
                    <h3 className="text-xs font-bold uppercase tracking-wider text-primary flex items-center gap-2">
                        {UI_TEXT.titles.arbitrageSignals}
                    </h3>
                    <div className="flex items-center gap-2">
                        <Filter className="h-3 w-3 text-zinc-500" />
                        <span className="text-[10px] text-zinc-500 uppercase">{UI_TEXT.labels.minProfit}</span>
                        <Input
                            type="number"
                            step="0.01"
                            value={minProfit}
                ...
                <Table>
                    <TableHeader className="bg-zinc-900/80 backdrop-blur">
                        <TableRow className="hover:bg-transparent border-zinc-800">
                            <TableHead className="w-[100px] text-zinc-400">{UI_TEXT.labels.pair}</TableHead>
                            <TableHead className="text-zinc-400">{UI_TEXT.labels.profit}</TableHead>
                            <TableHead className="text-zinc-400">{UI_TEXT.labels.buyAt}</TableHead>
                            <TableHead className="text-zinc-400">{UI_TEXT.labels.sellAt}</TableHead>
                            <TableHead className="text-right text-zinc-400">{UI_TEXT.labels.time}</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {filteredData.length === 0 ? (
                            <TableRow>
                                <TableCell colSpan={5} className="h-24 text-center text-zinc-500 italic">
                                    {data.length > 0 ? UI_TEXT.labels.noSignalsMatch : UI_TEXT.labels.noSignals}
                                </TableCell>
                            </TableRow>
                        ) : (
