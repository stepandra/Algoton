...
func (s *FuturesScanner) handleWebSocket(w http.ResponseWriter, r *http.Request) {
	log.Printf("Попытка WebSocket-подключения от %s", r.RemoteAddr)

	conn, err := s.upgrader.Upgrade(w, r, nil)
	if err != nil {
		log.Printf("Ошибка апгрейда WebSocket от %s: %v", r.RemoteAddr, err)
		return
	}
	defer conn.Close()
	...
	log.Printf("WebSocket клиент подключился от %s. Клиентов: %d", r.RemoteAddr, clientCount)

	defer func() {
		s.clientsMutex.Lock()
		delete(s.wsClients, conn)
		log.Printf("WebSocket клиент отключился. Клиентов: %d", len(s.wsClients))
		s.clientsMutex.Unlock()
	}()
...
func main() {
	...
	http.HandleFunc("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`{"status":"ok","version":"algoton-v1"}`))
	})
